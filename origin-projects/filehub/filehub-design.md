...
CREATE TABLE folders (
    ...
    folder_id       VARCHAR(12) UNIQUE NOT NULL,      -- 随机ID (base62 12位)
    name            VARCHAR(255) NOT NULL,            -- 文件夹名称
    parent_id       VARCHAR(12),                      -- 父文件夹ID (NULL表示根目录)
    created_by      VARCHAR(64) NOT NULL,             -- 创建者
    created_at      DATETIME DEFAULT CURRENT_TIMESTAMP,
    updated_at      DATETIME DEFAULT CURRENT_TIMESTAMP,
    UNIQUE(name, parent_id)                          -- 同一父目录下名称唯一
);
...
CREATE TABLE files (
    ...
    folder_id       VARCHAR(12),                      -- 所属文件夹ID (NULL表示根目录)
...
Request:
{
  "folder_id": "folder123"      -- 目标文件夹ID (null表示移动到根目录)
}
...
Response 200:
{
  "code": 0,
  "data": {
    "url": "https://filehub.example.com/app/files/aB3dE9kLmN0P"
  }
}
```
...
Request:
{
  "name": "Documents",              -- 文件夹名称
  "parent_id": "folder123"          -- 父文件夹ID (不传则在根目录创建)
}
...
#### 列出文件夹
```
GET /api/v1/folders?parent_id=folder123
Authorization: Bearer xxx
...
Response 200:
{
  ...
  "data": {
    ...
    "files": [
      {
        ...
        "created_at": "2026-02-12T10:30:00Z",
        "download_url": "https://xxx/api/v1/files/aB3dE9kLmN0P/download",
        "view_url": "https://xxx/app/files/aB3dE9kLmN0P"
      }
    ],
    "stats": {
      "folder_count": 1,
    ...
    "breadcrumbs": [
      { "folder_id": null, "name": "Root" },
      { "folder_id": "folder123", "name": "Work" }
    ]
  }
}
```
...
Request:
{
  "parent_id": "folder789"          -- 目标父文件夹ID (null表示移动到根目录)
}
...
#### 获取文件夹访问链接
```
GET /api/v1/folders/{folder_id}/view
Authorization: Bearer xxx
...
Response 200:
{
  "code": 0,
  "data": {
    "url": "https://filehub.example.com/app/folders/folder123"
  }
}
```
...
#### 文件上传（支持指定文件夹）
```
POST /api/v1/files?folder_id=folder123
Content-Type: multipart/form-data
Authorization: Bearer xxx
...
#### 文件列表（按文件夹）
```
GET /api/v1/files?folder_id=folder123&limit=20&offset=0
Authorization: Bearer xxx
...
```bash
# 上传文件到指定文件夹
filehub-cli upload /path/to/local/file.pdf --folder folder123
# 输出: filehub://aB3dE9kLmN0P

# 上传到根目录
filehub-cli upload /path/to/file.pdf

# 下载文件
filehub-cli download filehub://aB3dE9kLmN0P --output ./downloads/

# 列出指定文件夹的文件
filehub-cli ls-files --folder folder123

# 列出根目录文件
filehub-cli ls-files

# 移动文件到文件夹
filehub-cli mv-file aB3dE9kLmN0P --to folder123
...
# 获取文件访问页面链接（需登录）
filehub-cli url-file aB3dE9kLmN0P
```
...
# 创建文件夹（在指定父文件夹）
filehub-cli mkdir Projects --parent folder123

# 列出文件夹
filehub-cli ls-folders --parent folder123

# 列出根目录文件夹
filehub-cli ls-folders

# 列出文件夹内容（文件+子文件夹）
filehub-cli ls --folder folder123

# 重命名文件夹
filehub-cli rename-folder folder123 "NewName"

# 移动文件夹
filehub-cli mv-folder folder123 --to folder456

# 删除空文件夹
filehub-cli rmdir folder123

# 获取文件夹访问页面链接（需登录）
filehub-cli url-folder folder123
```
...
```bash
# 查看文件详情
filehub-cli stat filehub://aB3dE9kLmN0P

# 查看文件夹详情
filehub-cli stat-folder folder123

# 搜索文件（在当前文件夹及子文件夹）
filehub-cli find "*.pdf" --folder folder123

# 递归上传目录（保持结构）
filehub-cli upload /path/to/dir --recursive --folder folder123
```
...
|------|------|------|
| `/` | 文件列表 | 首页，根目录文件列表 |
| `/folder/{id}` | 文件夹内容 | 展示文件夹内的文件和子文件夹 |
...
├───────────────┬──────────────────────────────────────────────┤
│ Sidebar       │ Breadcrumb: Root > Documents > Work          │
│ Files         │ Toolbar: [New Folder] [Upload] | Sort | View │
│ Upload        │──────────────────────────────────────────────│
│ Activity      │ Folders:                                      │
│ Settings      │ ┌──────────────────────────────────────────┐ │
│               │ │ [Folder] Projects        3 items         │ │
│               │ │ [Folder] Archives        12 items        │ │
│               │ └──────────────────────────────────────────┘ │
│               │ Files:                                        │
│               │ ┌──────────────────────────────────────────┐ │
│               │ │ [Icon] report.pdf   12MB  2026-02-12 ... │ │
│               │ └──────────────────────────────────────────┘ │
│               │ ┌──────────────────────────────────────────┐ │
│               │ │ [Icon] photo.jpg     3MB  2026-02-11 ... │ │
│               │ └──────────────────────────────────────────┘ │
└───────────────┴──────────────────────────────────────────────┘
```
...
### Phase 1.5: 文件夹功能
- [ ] 数据库表结构更新 (folders 表, files 表添加 folder_id)
- [ ] 后端 API: 文件夹 CRUD 接口
- [ ] 后端 API: 文件移动到文件夹
- [ ] CLI: 文件夹管理命令 (mkdir, ls-folders, rmdir, mv-folder)
- [ ] CLI: 文件操作支持 folder 参数
- [ ] Web UI: 文件夹浏览界面
- [ ] Web UI: 新建/重命名/删除文件夹
- [ ] Web UI: 文件移动到文件夹
- [ ] 面包屑导航
