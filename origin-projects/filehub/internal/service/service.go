...
func (s *Service) Upload(ctx context.Context, header *multipart.FileHeader, createdBy string, folderID *string) (db.FileRecord, error) {
	...
	if err := s.DB.CreateFileWithFolder(ctx, record); err != nil {
		return db.FileRecord{}, err
	}
	return record, nil
}
...
func (s *Service) ListFiles(ctx context.Context, limit, offset int, order, keyword string, folderID *string) ([]db.FileRecord, int, error) {
	return s.DB.ListFilesByFolder(ctx, folderID, limit, offset, order, keyword)
}
