...
func (h *Handler) UploadFile(c *gin.Context) {
	...
	filePath, _ := h.buildFilePath(c.Request.Context(), record.FileID)
	shareURL := h.buildShareDownloadURL(c, record.FileID)
	OK(c, gin.H{
		"file_id":       record.FileID,
		"filehub_url":   h.buildFilehubURL(filePath),
		"path":          filePath,
...
func (h *Handler) GetFile(c *gin.Context) {
	...
	filePath, _ := h.buildFilePath(c.Request.Context(), fileID)
	shareURL := h.buildShareDownloadURL(c, fileID)
	OK(c, gin.H{
		"file_id":       record.FileID,
		"original_name": record.OriginalName,
		"path":          filePath,
...
func (h *Handler) buildFilePath(ctx context.Context, fileID string) (string, error) {
	return h.Service.DB.GetFilePath(ctx, fileID)
}

func (h *Handler) buildFilehubURL(path string) string {
	if path == "" {
		return "filehub:/"
	}
	return "filehub:" + path
...
func (h *Handler) GetFileByPath(c *gin.Context) {
	...
	file, err := h.Service.DB.GetFileByPath(c.Request.Context(), path)
	if err != nil {
		log.Printf("[GetFileByPath] 文件不存在: path=%s: %v", path, err)
		Error(c, http.StatusNotFound, 10003, "file not found")
		return
	}

	filePath, _ := h.buildFilePath(c.Request.Context(), file.FileID)
...
func (h *Handler) DownloadFileByPath(c *gin.Context) {
	path := c.Query("path")
	if path == "" {
		Error(c, http.StatusBadRequest, 10004, "path is required")
		return
	...
	file, err := h.Service.DB.GetFileByPath(c.Request.Context(), path)
	if err != nil {
		log.Printf("[DownloadFileByPath] 文件不存在: path=%s: %v", path, err)
		Error(c, http.StatusNotFound, 10003, "file not found")
		h.audit(c, "download", "", getUser(c), "failure", "not found")
		return
	}

	if err := h.streamObject(c, file, false); err != nil {
		Error(c, http.StatusInternalServerError, 10006, "download failed")
		h.audit(c, "download", file.FileID, getUser(c), "failure", "stream error")
		return
	}
	h.audit(c, "download", file.FileID, getUser(c), "success", "")
}
