...
func (h *Handler) ListFolders(c *gin.Context) {
	...
	for _, record := range records {
		folderPath, _ := h.Service.DB.GetFolderPath(c.Request.Context(), record.FolderID)
...
func (h *Handler) GetFolderContents(c *gin.Context) {
	...
	for _, file := range files {
		filePath, _ := h.Service.DB.GetFilePath(c.Request.Context(), file.FileID)
		shareURL := h.buildShareDownloadURL(c, file.FileID)
		fileResponses = append(fileResponses, gin.H{
			...
			"view_url":      h.buildBaseURL(c) + "/app/f" + filePath,
		})
	}
	...
	for _, f := range folders {
		folderPath, _ := h.Service.DB.GetFolderPath(c.Request.Context(), f.FolderID)
		itemCount, _ := h.Service.DB.GetFolderItemCount(c.Request.Context(), f.FolderID)
	...
	folderPath, _ := h.Service.DB.GetFolderPath(c.Request.Context(), folder.FolderID)
...
func (h *Handler) DeleteFolder(c *gin.Context) {
	...
	log.Printf("[DeleteFolder] 开始删除文件夹: %s, recursive=%v", folderID, recursive)
	...
	if itemCount > 0 && recursive {
		log.Printf("[DeleteFolder] 开始递归删除文件夹内容")
		if err := h.deleteFolderRecursive(c.Request.Context(), folderID); err != nil {
			log.Printf("[DeleteFolder] 递归删除失败: %v", err)
			Error(c, http.StatusInternalServerError, 19999, "recursive delete failed: "+err.Error())
			return
		}
		log.Printf("[DeleteFolder] 递归删除完成")
	}
...
func (h *Handler) deleteFolderRecursive(ctx context.Context, folderID string) error {
	for {
		files, _, err := h.Service.DB.ListFilesByFolder(ctx, &folderID, 1000, 0, "desc", "")
		if err != nil {
		...
		for _, f := range files {
			if _, err := h.Service.DB.DeleteFile(ctx, f.FileID); err != nil {
				return fmt.Errorf("delete file %s failed: %w", f.FileID, err)
			}
		}
	}

	for {
		folders, err := h.Service.DB.ListFolders(ctx, &folderID)
		if err != nil {
			return fmt.Errorf("list subfolders failed: %w", err)
		}
		if len(folders) == 0 {
			break
		}
		for _, f := range folders {
			if err := h.deleteFolderRecursive(ctx, f.FolderID); err != nil {
				return err
			}
			if err := h.Service.DB.DeleteFolder(ctx, f.FolderID); err != nil {
				return fmt.Errorf("delete subfolder %s failed: %w", f.FolderID, err)
			}
		}
	}
...
func (h *Handler) GetFolderByPath(c *gin.Context) {
	path := c.Query("path")
	if path == "" {
	...
	folder, err := h.Service.DB.GetFolderByPath(c.Request.Context(), path)
	if err != nil {
		log.Printf("[GetFolderByPath] 文件夹不存在: path=%s: %v", path, err)
		Error(c, http.StatusNotFound, 10003, err.Error())
		return
	}
