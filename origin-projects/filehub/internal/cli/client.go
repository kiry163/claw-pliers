...
// DeleteFolder 删除文件夹
func (c *Client) DeleteFolder(folderID string, recursive bool) error {
	url := c.Endpoint + "/api/v1/folders/" + folderID
	if recursive {
		url += "?recursive=true"
	}
...
func (c *Client) GetFolderByPath(path string) (*FolderPathResponse, error) {
	...
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("get folder by path failed: %s", resp.Status)
	}
	var payload APIResponse
	if err := json.NewDecoder(resp.Body).Decode(&payload); err != nil {
		return nil, err
...
func (c *Client) GetFileByPath(path string) (*FileItem, error) {
	...
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("get file by path failed: %s", resp.Status)
	}
	var payload APIResponse
	if err := json.NewDecoder(resp.Body).Decode(&payload); err != nil {
		return nil, err
...
func (c *Client) DownloadFileByPath(path, outputPath string, progress func(int)) (string, error) {
	url := c.Endpoint + "/api/v1/files/by-path/download?path=" + path
	req, err := http.NewRequest("GET", url, nil)
	if err != nil {
		return "", err
	}
...
func (c *Client) GetRootContents() (*FolderContents, error) {
	folders, err := c.ListFolders(nil)
	if err != nil {
		return nil, fmt.Errorf("list root folders failed: %w", err)
	}

	files, _, err := c.ListFiles(nil, 1000, 0, "desc", "")
	if err != nil {
		return nil, fmt.Errorf("list root files failed: %w", err)
	}
	...
	return &FolderContents{
		...
		Breadcrumbs: []BreadcrumbItem{{FolderID: nil, Name: "Root"}},
	}, nil
}

// MoveFile 移动文件到文件夹
func (c *Client) MoveFile(fileID string, folderID *string) error {
	body := map[string]interface{}{}
	if folderID != nil {
	...
	req, err := http.NewRequest("PUT", c.Endpoint+"/api/v1/files/"+fileID+"/move", bytes.NewReader(data))
	if err != nil {
		return err
	}
	req.Header.Set("Content-Type", "application/json")
	...
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		return fmt.Errorf("move file failed: %s", resp.Status)
	}
	return nil
}
